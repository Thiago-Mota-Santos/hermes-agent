...
export function KanbanDetail({ taskId, board, profiles, now, onClose, onChanged }: KanbanDetailProps) {
  ...
  if (!detail) {
    return (
      <aside className="shadow-nous flex h-full w-96 shrink-0 items-center justify-center border-l border-(--stroke-nous) bg-(--ui-chat-surface-background)">
  ...
  async function viewLog() {
    try {
      const result = await getKanbanTaskLog(taskId, 4000, board)
  ...
  return (
    <aside className="shadow-nous flex h-full w-96 shrink-0 flex-col border-l border-(--stroke-nous) bg-(--ui-chat-surface-background)">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-4 pb-4">
        ...
        {detail.runs.length > 0 ? (
          <div className="flex flex-col gap-1.5">
            ...
            {log !== null ? (
              <pre className="max-h-48 overflow-auto rounded-[6px] border border-(--ui-stroke-tertiary) bg-(--ui-bg-quaternary) p-2 text-[0.62rem] leading-snug text-(--ui-text-secondary)">
                {log}
              </pre>
            ) : null}
          </div>
